        ...
        UserRelationship.findOne({username: req.query.username}, (err, result) =>
            {
                if(!err)
                {
                    ...
                    UsersAlgorithmView.findOne({ username: req.query.username }, (err, result) =>
                    {
                        //console.log(result);
                        ...
                        UsersAlgorithmView.find({ $and: [ { username: { $nin: tmpList } }, { location: { $in: result.listPrefLoc }}, { listPrefLoc: result.location }, { gender: { $in: result.listGenders }}, { listGenders: result.gender }, { listInterests: { $nin: result.listTurnOffs } }, { listTurnOffs: { $nin: result.listInterests } }, { $or: [ { $and: [ { listInterests: { $in: result.listTurnOns } }, { listTurnOns: { $in: result.listInterests } } ] }, { $and: [ { listInterests: { $in: result.listInterests } } ] } ] } ] }, (err, potentialPartners) =>
                        {
                            //console.log(result)

                            usernameList = []

                            potentialPartners.forEach(
                                y =>
                            ...
                            UserRelationship.find({ $and: [ { username: { $in: usernameList }}, { listBlocked: { $ne: req.query.username } }, { listLeftSwipes: { $ne: req.query.username } } ] }, { username: 1, _id: 0 }, (err, result) =>
                            {
                                //console.log(result)
